...
2. **Use validation in frontends** — Optional: call the same Zod schemas (or thin wrappers) in vue-app / nuxt-app forms before submit.

3. **Nuxt SSR (when needed)** — Route-by-route SSR and server-side cookie/session forwarding for `bootstrapSession`; keep current SPA mode until SEO or first-hit auth matters.

4. **UI parity** — Align Nuxt screens with vue-app (e.g. PrimeVue / shared component kit) if both apps stay product-facing.

5. **`turbo.json` (optional)** — Faster pipelines and clearer task graph once package/app count grows.

6. **Future apps** — Placeholders only until scoped: admin SPA, mobile client, etc.
